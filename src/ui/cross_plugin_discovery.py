...
conversion (#47). If the sibling is installed, activate its dock; if not,
...
def make_ai_seg_action(parent, iface, label: str, tooltip: str,
    ...
    """Create a QAction that opens AI Segmentation if installed, else the product page."""
    ...
    def triggered():
        ...
        if plugin is not None and _activate_dock(plugin):
            return
        QDesktopServices.openUrl(QUrl(_AI_SEG_PRODUCT_URL))
