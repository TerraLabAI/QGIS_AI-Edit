"""The plan as the Account and Billing pages read it, from one account payload.

``client.get_account(include_usage=True)`` answers with the email, the
subscriptions (one per product) and the ``usage`` block. Both pages paint from
``AccountPlan``, so they can never disagree on how much is left.
"""
...
def find_product_subscription(data: dict, product_id: str) -> dict | None:
    """``product_id`` is account_settings_dialog.PRODUCT_ID, passed in because
...
def resolve_account_plan(data: dict, product_id: str) -> AccountPlan:
    """The subscription first, the bundled usage block for what it lacks."""
