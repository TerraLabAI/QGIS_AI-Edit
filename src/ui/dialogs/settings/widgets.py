"""Building blocks of the settings dialog, ported from AI Agent's
``settings_pages.py``: the page frame, the setting row and group, the pill
switch, the plan card and the rail's stylesheet.

Every colour is a token from ``dock/design_tokens.py``, so the dark and the
light QGIS theme read the same way. The interaction colour is AI Edit's blue
(hover wash, selected row), the one filled button is the Material green.
"""
...
# One card holding the rows, hairlines between them.
_GROUP_QSS = (
...
def nav_qss(name: str, category: str | None = None) -> str:
    """The rail: rows with 8 px corners, the blue wash under the pointer. The
    selected row wears the tint of its own page's hue (``category``), the
    blue wash when the page has none."""
...
class Switch(QAbstractButton):
    """AI Agent's pill switch: ink track when on, a faint track when off."""
...
class SettingRow(QFrame):
    """A title, an optional muted note, and a control at the right (or below)."""
    ...
    def __init__(self, title: str, note: str = "", control: QWidget | None = None,
        ...
        # A switch has no text: give it the row's words for a screen reader.
        if control is not None:
            try:
...
class SettingGroup(QFrame):
    """The rounded card that stacks rows with hairlines between them.

    ``category`` tints it (the danger zone is coral): a line in the hue and a
    faint ground; the words keep their inks."""
...
class Page(QWidget):
    """Title, one muted subtitle, then a scrolling column of cards.

    ``glyph`` and ``category`` put the page's icon tile left of the title, in
    the hue its rail row wears."""
    ...
    def __init__(self, title: str, subtitle: str, parent=None,
        ...
        if glyph:
            # Under the title's first letter, not under the tile.
...
def make_button(text: str, qss: str, parent=None) -> QPushButton:
    """A pill that keeps its own width: the verb, never a full-width bar."""
...
def gauge_qss(fraction_left: float) -> str:
    """A remaining-allowance groove; the chunk takes the gauge hue."""
...
def make_usage_bar(value: int, maximum: int, parent=None) -> QProgressBar:
    """What is left of the month: leaf green, amber under a fifth, coral once
    nothing is left (``gauge_category``). The figure above says it in words."""
...
# Below this dialog width the two cards stack.
BILLING_STACK_W = 720
...
class BillingCard(QFrame):
    """One card of the Billing page: a micro title, then what the caller stacks.

    ``category`` gives the micro title and the big figure the hue's ink."""
    ...
    def __init__(self, title: str, parent=None, category: str | None = None):
        ...
        # Everything stacks above this, so a short card stays short at the top.
    ...
    def add_point(self, text: str, check_color: str) -> QWidget:
        """One feature line: a check in the given colour, then the words."""
    ...
    def add_action(self, button: QPushButton, qss: str) -> QPushButton:
        """A full-width button of the card."""
        button.setStyleSheet(qss)
        button.setCursor(Qt.CursorShape.PointingHandCursor)
        button.setAutoDefault(False)
        row = QHBoxLayout()
...
class BillingCardRow(QWidget):
    """Two cards side by side on a wide dialog, stacked on a narrow one."""
    ...
    def _watch_viewport(self) -> None:
        """Follow the page's scroll viewport: side by side, the cards hold this
        row at their summed minimum, so its own width stops moving."""
    ...
    def _room(self) -> int:
        """Width the row may take without pushing the page sideways."""
        viewport = getattr(self, "_viewport", None)
        if viewport is None:
            return self.width()
        left = self.mapTo(viewport.parentWidget(), self.rect().topLeft()).x() - viewport.x()
        # The page margins are symmetric: the same gap sits on the right.
